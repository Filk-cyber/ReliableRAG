...
def get_llama3_generate_reasoning_chains_prompts_chat_format(
    ...
    for triples, candidates in zip(existing_triples, candidate_triples):
        ...
        prompts.append(
            [  # instruction: first give the requirements, then the examplars and known conditions, and finally provide the blank 'the next possible triple is:' in the user content
...
def construct_reasoning_chains(args, ideal_setting: bool = False):
    ...
    for example in tqdm(data, desc="Generating reasoning chains", total=len(data)):
            ...
            for i in range(len(paths)):
                ...
                for b in range(args.num_beams):
                    ...
                    if current_choice == 'A':
                        ...
                        # Add the selected triple to the path, record probability index
                        new_paths.append(
        ...
        def get_single_answer_log_probability(model, tokenizer, input_ids, attention_mask, answer_text):
            ...
            try:
                answer_tokens = tokenizer(answer_text, return_tensors="pt", add_special_tokens=False)["input_ids"][
                    0]
            except:
                return float('-inf')
            ...
            answer_tokens = answer_tokens.to(current_input_ids.device)
            ...
            with torch.no_grad():
                for i, target_token_id in enumerate(answer_tokens):
                    ...
                    target_token_tensor = target_token_id.unsqueeze(0).unsqueeze(0)
        ...
        def get_multiple_answers_max_log_prob(model, tokenizer, prompt_text, correct_answers):
            """Calculate the max log_prob among multiple correct answers predicted by the model"""

            # 1. Build chat format messages
            messages = [
                {
                    "role": "system",
            ...
            # 2. Convert to model input using apply_chat_template
        ...
        # 1. Get correct answer list
        raw_answers = example.get("answers", [])

        if raw_answers and len(paths) > 0:

            # 2. Baseline Logic (Facts="", Context="")
            baseline_prompt = f"""Given the following information: 
    ...
    Answer: """
            ...
            # Get max Log Prob of Baseline
            baseline_log_prob = get_multiple_answers_max_log_prob(model, tokenizer, baseline_prompt, raw_answers)

            entropy_scores = []

            # 3. Chain Logic (Facts=Chain, Context="")
            for path in paths:
                # Build reasoning chain text
                chain_triples_text_list = []
                for triple_index in path:
                    if triple_index >= 0:
                        triple_raw = triples[triple_index]
                        triple_clean = triple_raw.replace("<", "").replace(">", "").replace(";", "", 2)
                        if triple_clean not in chain_triples_text_list:
                            chain_triples_text_list.append(triple_clean)
                facts_text = " ".join(chain_triples_text_list)

                prompt_with_chain = f"""Given the following information: 
    {facts_text}
    ...
    Answer:"""
                ...
                chain_log_prob = get_multiple_answers_max_log_prob(model, tokenizer, prompt_with_chain, raw_answers)

                # Calculate Delta (Log Prob increment)
                delta = chain_log_prob - baseline_log_prob
                entropy_scores.append(delta)

            # 4. Re-rank (Descending Sort)
            combined = list(zip(paths, paths_scores, paths_finished, entropy_scores))
